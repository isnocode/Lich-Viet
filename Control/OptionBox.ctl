...
Private Sub DrawOption(Optional ByVal State As Integer = 0)
   ...
   End If
    ...
    If DrawTheme("Button", 2, j, MRC) = False Then
        If Me.Value = False Then
            If vState = 1 Then
                j = DFCS_BUTTONRADIO Or DFCS_PUSHED
            ElseIf vState = 0 Then
                j = DFCS_BUTTONRADIO
            End If
         Else
            If vState = 1 Then
                j = DFCS_BUTTONRADIO Or DFCS_CHECKED Or DFCS_PUSHED
            ElseIf vState = 0 Then
                j = DFCS_BUTTONRADIO Or DFCS_CHECKED
            End If
          End If
...
Public Property Get Default() As OLE_OPTEXCLUSIVE
    Default = m_Value
End Property

Public Property Let Default(ByVal new_value As OLE_OPTEXCLUSIVE)
    Value = new_value
    DrawOption 0
...
Private Sub UserControl_MouseUp(Button As Integer, Shift As Integer, x As Single, y As Single)
    If MDMouseOver(hwnd) = True Then
        If Me.Value <> True Then Me.Value = True
        RaiseEvent Click
    End If
    Timer1.Enabled = True
    Timer1.Interval = 1
