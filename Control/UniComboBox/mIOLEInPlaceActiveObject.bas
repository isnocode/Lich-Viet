Attribute VB_Name = "cbo_mIOLEInPlaceActiveObject"
Option Explicit
...
Public Const S_OK = 0
Public Type IPAOHookStructComboBox 'IOleInPlaceActiveObjectHook
    lpVTable As Long 'VTable pointer
    IPAOReal As IOleInPlaceActiveObject 'Un-AddRefed pointer for forwarding calls
    TBEx As UniComboBox   'Un-AddRefed native class pointer for making Friend calls
    ThisPointer As Long
End Type
...
Public Property Get cboIPAOVTable() As Long
...
Private Function AddRef(This As IPAOHookStructComboBox) As Long
   ' Call the UserControl's standard AddRef method:
   AddRef = This.IPAOReal.AddRef
End Function

Private Function Release(This As IPAOHookStructComboBox) As Long
   ' Call the UserControl's standard Release method:
   Release = This.IPAOReal.Release
End Function

Private Function QueryInterface(This As IPAOHookStructComboBox, riid As GUID, pvObj As Long) As Long
...
Private Function GetWindow(This As IPAOHookStructComboBox, phwnd As Long) As Long
   ' Call user controls' GetWindow method:
   GetWindow = This.IPAOReal.GetWindow(phwnd)
End Function

Private Function ContextSensitiveHelp(This As IPAOHookStructComboBox, ByVal fEnterMode As Long) As Long
   ' Call the user control's ContextSensitiveHelp method:
   ContextSensitiveHelp = This.IPAOReal.ContextSensitiveHelp(fEnterMode)
End Function

Private Function TranslateAccelerator(This As IPAOHookStructComboBox, lpMsg As Msg) As Long
Dim hRes As Long
...
Private Function OnFrameWindowActivate(This As IPAOHookStructComboBox, ByVal fActivate As Long) As Long
...
Private Function OnDocWindowActivate(This As IPAOHookStructComboBox, ByVal fActivate As Long) As Long
   ' Call the user control's OnDocWindow activate interface:
   OnDocWindowActivate = This.IPAOReal.OnDocWindowActivate(fActivate)
End Function

Private Function ResizeBorder(This As IPAOHookStructComboBox, prcBorder As RECT, ByVal puiWindow As IOleInPlaceUIWindow, ByVal fFrameWindow As Long) As Long
...
Private Function EnableModeless(This As IPAOHookStructComboBox, ByVal fEnable As Long) As Long
